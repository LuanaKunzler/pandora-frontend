...
export class InterestedComponent implements OnInit {
  ...
  @ViewChild('interestedCards')
  interestedCards!: ElementRef;

  paramSubscription: Subscription = new Subscription;

  showcaseState: Observable<ShowcaseState> = new Observable<ShowcaseState>();
  ...
  ngOnInit() {
    ...
    this.store.select('authorization').pipe(take(1)).subscribe(auth => {
